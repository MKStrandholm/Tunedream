    ...
    try {
      ...
      try {
        const { error } = await supabase
          .from('songs')
          .upsert(
            { title: title, image: fullB64String }, // row to insert
            { onConflict: 'title' }                 // avoid duplicates by title
          );

        if (error) {
          console.log(`Failed to insert [${title}] into db: ${error.message}`);
        ...
        console.log(`Unexpected error inserting [${title}]: ${dbErr.message}`);
